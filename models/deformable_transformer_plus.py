...
from models import WeightAttention
...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, num_proposals, query_embed=None, ref_pts=None, mem_bank=None, mem_bank_pad_mask=None, attn_mask=None):
        ...
        if self.two_stage:
            ...
            init_p_ref_pts = reference_points[:, :num_proposals]
            ...
            init_p_ref_pts = reference_points[:, :num_proposals]
        # decoder
        p_hs, inter_p_ref_pts, pt_weights_cos, pp_weights_cos, pt_weights_mm, pp_weights_mm = \
            self.decoder(tgt, reference_points, memory,spatial_shapes, level_start_index, valid_ratios,
                         num_proposals, mask_flatten,mem_bank, mem_bank_pad_mask, attn_mask)
        if self.two_stage:
            raise NotImplementedError
            return p_hs, init_p_ref_pts, inter_p_ref_pts, enc_outputs_class, enc_outputs_coord_unact
        return p_hs, init_p_ref_pts, inter_p_ref_pts, pt_weights_cos, pp_weights_cos, pt_weights_mm, pp_weights_mm
